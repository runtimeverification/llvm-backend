...
llvm::Type *getParamType(ValueType sort, llvm::Module *Module) {
  ...
  case SortCategory::Set:
    type = llvm::PointerType::get(type, 1);
    break;
  default:
    break;
  }
...
static llvm::Value *addrspaceCast(llvm::Value *val, llvm::BasicBlock *block, int from, int to) {
  ...
  auto malloc = llvm::CallInst::Create(
      ...
      llvm::ConstantExpr::getPtrToInt(
        llvm::ConstantExpr::getGetElementPtr(
          AllocType, 
          llvm::ConstantPointerNull::get(llvm::PointerType::get(AllocType, 1)), 
          llvm::ConstantInt::get(llvm::Type::getInt32Ty(block->getContext()), 1)),
...
llvm::Value *CreateTerm::createToken(ValueType sort, std::string contents) {
  ...
  case SortCategory::Int: {
    ...
    return addrspaceCast0to1(llvm::ConstantExpr::getInBoundsGetElementPtr(getTypeByName(Module, INT_WRAPPER_STRUCT), globalVar, Idxs), CurrentBlock);
  }
  case SortCategory::Float: {
    ...
    std::vector<llvm::Constant *> Idxs = {llvm::ConstantInt::get(llvm::Type::getInt64Ty(Ctx), 0), llvm::ConstantInt::get(llvm::Type::getInt32Ty(Ctx), 1)};
    return addrspaceCast0to1(llvm::ConstantExpr::getInBoundsGetElementPtr(getTypeByName(Module, FLOAT_WRAPPER_STRUCT), globalVar, Idxs), CurrentBlock);
  }
  case SortCategory::StringBuffer:
  ...
  case SortCategory::Symbol: {
    ...
    return llvm::ConstantExpr::getPointerCast(global, llvm::PointerType::get(getTypeByName(Module, BLOCK_STRUCT), 1));
  }
  case SortCategory::Uncomputed:
    abort();
  }
...
// we use tailcc calling convention for apply_rule_* and eval_* functions so that we
// can add the musttail marker to these calls so the calls become tail calls
llvm::Value *CreateTerm::createFunctionCall(std::string name, KORECompositePattern *pattern, bool sret, bool tailcc) {
...
llvm::Value *CreateTerm::createFunctionCall(std::string name, ValueType returnCat, const std::vector<llvm::Value *> &args, bool sret, bool tailcc) {
...
std::pair<llvm::Value *, bool> CreateTerm::operator()(KOREPattern *pattern) {
  if (auto variable = dynamic_cast<KOREVariablePattern *>(pattern)) {
    ...
    if (symbolDecl->getAttributes().count("function") || (symbolDecl->getAttributes().count("anywhere") && !isAnywhereOwise)) {
      ...
      llvm::Value *Cast = llvm::CallInst::Create(getOrInsertFunction(Module, "inttoptr_i64.p1s_blocks", llvm::PointerType::get(BlockType, 1), llvm::Type::getInt64Ty(Ctx)), {llvm::ConstantInt::get(llvm::Type::getInt64Ty(Ctx), (((uint64_t)symbol->getTag()) << 32) | 1)}, "", CurrentBlock);
...
bool makeFunction(std::string name, KOREPattern *pattern, KOREDefinition *definition, llvm::Module *Module, bool tailcc, bool bigStep, KOREAxiomDeclaration *axiom, std::string postfix) {
