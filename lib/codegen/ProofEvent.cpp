...
llvm::CallInst *proof_event::emit_write_tail_call_info(
    ...
    std::string const &callee_name, bool is_tail, llvm::BasicBlock *insert_at_end) {
  auto b = llvm::IRBuilder(insert_at_end);
  ...
  auto *func_ty
      = llvm::FunctionType::get(void_ty, {i8_ptr_ty, i8_ptr_ty, i8_ptr_ty, i8_ty}, false);
  ...
  auto *var_callee_name
      = b.CreateGlobalStringPtr(callee_name, "", 0, module_);
  auto *var_is_tail = llvm::ConstantInt::get(i8_ty, is_tail);
  return b.CreateCall(func, {proof_writer, var_caller_name, var_callee_name, var_is_tail});
}
...
llvm::BasicBlock *proof_event::tail_call_info(
    std::string const &caller_name, std::string const &callee_name, bool is_tail,
  ...
  emit_write_tail_call_info(proof_writer, caller_name,callee_name, is_tail, true_block);
