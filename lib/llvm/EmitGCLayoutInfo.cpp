...
struct EmitGCLayoutInfo : public ModulePass {
  ...
  bool runOnModule(Module &M) override {
    ...
    Constant *C2 = M.getOrInsertGlobal(
        "gc_stackmap_num_relocations", Type::getInt32Ty(Ctx));
    auto G2 = cast<GlobalVariable>(C2);
    if (!G2->hasInitializer()) {
      G2->setInitializer(
          ConstantInt::get(Type::getInt32Ty(Ctx), numRelocations));
    }
...
static RegisterPass<EmitGCLayoutInfo>
    X("emit-gc-layout-info", "Emit Layout Info for Garbage Collection",
