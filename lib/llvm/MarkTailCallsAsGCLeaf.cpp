      ...
      if (auto R = dyn_cast<ReturnInst>(TI)) {
        ...
        if (auto CI = dyn_cast<CallInst>(BBI)) {
          ...
          CI->setAttributes(CI->getAttributes().addAttribute(
              Ctx, llvm::AttributeList::FunctionIndex, "gc-leaf-function"));
          dirty = true;
        }
      }
    }
...
static RegisterPass<MarkTailCallsAsGCLeaf>
    X("mark-tail-calls-as-gc-leaf",
      "Mark tailcc tail calls with gc-leaf-function attribute",
