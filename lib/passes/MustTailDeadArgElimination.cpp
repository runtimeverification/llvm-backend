...
bool DeadArgumentEliminationPass::deleteDeadVarargs(Function &F) {
  ...
  for (User *U : llvm::make_early_inc_range(F.users())) {
    ...
    if (!PAL.isEmpty()) {
      ...
      PAL = AttributeList::get(
          F.getContext(), PAL.getFnAttrs(), PAL.getRetAttrs(), ArgAttrs);
    }
    ...
    if (InvokeInst *II = dyn_cast<InvokeInst>(CB)) {
      NewCB = InvokeInst::Create(
          NF, II->getNormalDest(), II->getUnwindDest(), Args, OpBundles, "",
          CB);
    } else {
...
static unsigned numRetVals(Function const *F) {
...
static Type *getRetComponentType(Function const *F, unsigned Idx) {
  Type *RetTy = F->getReturnType();
...
DeadArgumentEliminationPass::markIfNotLive(
    RetOrArg Use, UseVector &MaybeLiveUses) {
...
/// it at 0.
DeadArgumentEliminationPass::Liveness DeadArgumentEliminationPass::surveyUse(
    Use const *U, UseVector &MaybeLiveUses, unsigned RetValNum) {
  User const *V = U->getUser();
  if (ReturnInst const *RI = dyn_cast<ReturnInst>(V)) {
    // The value is returned from a function. It's only live when the
    // function's return value is live. We use RetValNum here, for the case
    // that U is really a use of an insertvalue instruction that uses the
    // original Use.
    Function const *F = RI->getParent()->getParent();
    if (RetValNum != -1U) {
  ...
  if (InsertValueInst const *IV = dyn_cast<InsertValueInst>(V)) {
  ...
  if (auto const *CB = dyn_cast<CallBase>(V)) {
    Function const *F = CB->getCalledFunction();
    if (F) {
      ...
      assert(
          CB->getArgOperand(ArgNo) == CB->getOperand(U->getOperandNo())
...
DeadArgumentEliminationPass::Liveness DeadArgumentEliminationPass::surveyUses(
    Value const *V, UseVector &MaybeLiveUses) {
...
void DeadArgumentEliminationPass::surveyFunction(Function const &F) {
  ...
  for (BasicBlock const &BB : F) {
    ...
    // change
    if (auto TC = BB.getTerminatingMustTailCall())
      if (TC->getCallingConv() != CallingConv::Tail)
        HasMustTailCalls = true;
  }

  if (HasMustTailCalls) {
    LLVM_DEBUG(
        dbgs() << "DeadArgumentEliminationPass - " << F.getName()
               << " has musttail calls\n");
  }
  ...
  for (Use const &U : F.uses()) {
    ...
    // taken.
    auto const *CB = dyn_cast<CallBase>(U.getUser());
    if (!CB || !CB->isCallee(&U)
        || CB->getFunctionType() != F.getFunctionType()) {
      markLive(F);
      return;
    ...
    if (CB->isMustTailCall())
      if (CB->getCallingConv() != CallingConv::Tail)
      ...
      if (ExtractValueInst *Ext = dyn_cast<ExtractValueInst>(UU.getUser())) {
        ...
        for (unsigned Ri = 0; Ri != RetCount; ++Ri) {
          if (RetValLiveness[Ri] != Live)
            MaybeLiveRetUses[Ri].append(
                MaybeLiveAggregateUses.begin(), MaybeLiveAggregateUses.end());
        }
      }
    }
  }
  ...
  LLVM_DEBUG(
      dbgs() << "DeadArgumentEliminationPass - Inspecting args for fn: "
             << F.getName() << "\n");
...
void DeadArgumentEliminationPass::markValue(
    RetOrArg const &RA, Liveness L, UseVector const &MaybeLiveUses) {
  switch (L) {
  case Live: markLive(RA); break;
  case MaybeLive:
    assert(!isLive(RA) && "Use is already live!");
    for (auto const &MaybeLiveUse : MaybeLiveUses) {
...
void DeadArgumentEliminationPass::markLive(Function const &F) {
  LLVM_DEBUG(
      dbgs() << "DeadArgumentEliminationPass - Intrinsically live fn: "
             << F.getName() << "\n");
...
void DeadArgumentEliminationPass::markLive(RetOrArg const &RA) {
  if (isLive(RA))
  ...
  LLVM_DEBUG(
      dbgs() << "DeadArgumentEliminationPass - Marking " << RA.getDescription()
             << " live\n");
  propagateLiveness(RA);
}

bool DeadArgumentEliminationPass::isLive(RetOrArg const &RA) {
...
void DeadArgumentEliminationPass::propagateLiveness(RetOrArg const &RA) {
...
bool DeadArgumentEliminationPass::removeDeadStuffFromFunction(Function *F) {
  ...
  for (Function::arg_iterator I = F->arg_begin(), E = F->arg_end(); I != E;
    ...
    if (LiveValues.erase(Arg)) {
      ...
      LLVM_DEBUG(
          dbgs() << "DeadArgumentEliminationPass - Removing argument " << ArgI
  ...
  if (NRetTy->isVoidTy())
    ...
    assert(
        !RAttrs.overlaps(AttributeFuncs::typeIncompatible(NRetTy))
  ...
  while (!F->use_empty()) {
    ...
    ArgAttrVec.clear();
    AttributeList const &CallPAL = CB.getAttributes();
    ...
    if (!CB.use_empty() || CB.isUsedByMetadata()) {
      if (NewCB->getType() == CB.getType()) {
        ...
        assert(
            (RetTy->isStructTy() || RetTy->isArrayTy())
          ...
          for (unsigned RetI = 0; RetI != RetCount; ++RetI)
            if (NewRetIdxs[RetI] != -1) {
              ...
              if (RetTypes.size() > 1) {
                ...
                RetVal = IRB.CreateInsertValue(
                    RetVal, EV, NewRetIdxs[RetI], "newret");
              } else {
