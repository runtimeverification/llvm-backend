...
case class SymbolS() extends SortCategory {
  def hookAtt = ???
  def hasIncompleteSignature(sigma: Seq[Constructor], f: Fringe): Boolean = sigma.isEmpty || sigma.contains(Empty()) || sigma.size != f.length
...
abstract class EqualLiteral() extends SortCategory {
  ...
  private def tree(matrix: Matrix, ls: Seq[(String, Matrix)]): DecisionTree = {
    ...
    if (matrix.default.isDefined && ls.isEmpty) {
...
case class ListS() extends SortCategory {
  ...
  def tree(matrix: Matrix): DecisionTree = {
    ...
    Function("hook_LIST_size_long", newO, Seq(listO), "MINT.MInt 64",
      SwitchLit(newO, 64, matrix.cases.zipWithIndex.map(l => (l._1._1, expandListPattern(l._1._2, listO, matrix.sigma(l._2).asInstanceOf[ListC]))), matrix.default.map(expandListPatternDefault(_, listO, maxList))))
  }
}
...
case class BoolS() extends SortCategory {
  ...
  def hasIncompleteSignature(sigma: Seq[Constructor], f: Fringe): Boolean = f.length != 2
...
case class MIntS(bitwidth: Int) extends SortCategory {
  ...
  def hasIncompleteSignature(sigma: Seq[Constructor], f: Fringe): Boolean = f.length != (1 << bitwidth)
