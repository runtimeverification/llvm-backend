...
appendTests = testGroup "Basic pattern compilation"
  ...
  , testCase "Naive compilation of the append pattern with side condition" $
      compilePattern appendCondPattern @?=
        switch ["1"] [ ("nil", leaf 1 [["2"]])
               , ("cons", (switch ["2"] [ ("nil", leaf 2 [["1"]])
                                   , ("cons", (function "side_condition_3" ["side_condition_3"] [["1", "2"], ["0", "1"]] "BOOL.Bool" (switchLiteral ["side_condition_3"] 1 [("1", leaf 3 [["0", "2"], ["1", "2"], ["0", "1"], ["1", "1"]]), ("0", failure)] Nothing)))
                                   ] Nothing ))
               ] Nothing
