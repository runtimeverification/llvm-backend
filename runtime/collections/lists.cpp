...
SortKItem hook_LIST_get64(SortList list, uint64_t index) {
  if (index >= list->size()) {
    KLLVM_HOOK_INVALID_ARGUMENT(
        "Index out of range for get64: index={}, size={}", index, list->size());
  }
...
list hook_LIST_update64(SortList list, uint64_t idx, SortKItem value) {
  if (idx >= list->size()) {
    KLLVM_HOOK_INVALID_ARGUMENT(
        "Index out of range for update64: index={}, size={}", idx, list->size());
  }
