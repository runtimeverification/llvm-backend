...
```k
  syntax Pgm ::= "float" Ids ";" Stmt           [format(%1 %2%3%n%4), colors(yellow,pink)]
...
```k
  rule I1 / I2 => I1 /Float I2  requires I2 =/=Float 0.0
  rule I1 + I2 => I1 +Float I2
  rule - I1 => 0.0 -Float I1
```
...
```k
  rule <k> X = I:Float; => . ...</k> <state>... X |-> (_ => I) ...</state>
```
...
```k
  rule <k> float (X,Xs => Xs);_ </k> <state> Rho:Map (.Map => X|->0.0) </state>
    requires notBool (X in keys(Rho))
  rule float .Ids; S => S  [structural]
endmodule
```
