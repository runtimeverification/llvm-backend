...
BOOST_AUTO_TEST_CASE(open) {
  int fd = overwrite_test_file();
  ::close(fd);

  auto realFilename = make_string("test.txt");
  auto fakeFilename = make_string("testFake.txt");
  auto control = make_string("r");

  block *b1 = hook_IO_open(realFilename, control);
  BOOST_CHECK_EQUAL(
      b1->h.hdr, get_block_header_for_symbol(
                     get_tag_for_symbol_name("inj{SortInt{}, SortIOInt{}}"))
                     .hdr);
  BOOST_CHECK(0 < mpz_cmp_si((mpz_ptr) * (b1->children), 0));

  block *b2 = hook_IO_open(fakeFilename, control);
  BOOST_CHECK_EQUAL(
      b2->h.hdr, get_block_header_for_symbol(
                     get_tag_for_symbol_name("inj{SortIOError{}, SortKItem{}}"))
                     .hdr);
  BOOST_CHECK_EQUAL(
      (uint64_t) * (b2->children),
      ERRBLOCK(get_tag_for_symbol_name(GETTAG(ENOENT))));
}
...
BOOST_AUTO_TEST_CASE(close) {
  mpz_t f1;
  mpz_t f2;
  int fd1 = overwrite_test_file();
  mpz_init_set_si(f1, fd1);
  int fd2 = overwrite_test_file();
...
BOOST_AUTO_TEST_CASE(write) {
  mpz_t f;
  string *msg = make_string("This is a test message\n");
  int fd = overwrite_test_file();
  mpz_init_set_si(f, fd);
...
BOOST_AUTO_TEST_CASE(log) {
  ...
  string *path = make_string(strPath.c_str());
  string *msg = make_string(strMsg.c_str());
...
BOOST_AUTO_TEST_CASE(system) {
  ...
  string *cmd = make_string(command.c_str());
...
BOOST_AUTO_TEST_CASE(argv) {
  ...
  BOOST_CHECK_EQUAL(
      ret[0].elem->h.hdr,
      get_block_header_for_symbol(
          get_tag_for_symbol_name("inj{SortString{}, SortKItem{}}"))
          .hdr);
